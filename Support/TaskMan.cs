...
{
    /// <summary>
    /// Handles anything to do with Tasks (adopted from Microsoft's metadata "public class Task : IThreadPoolWorkItem, IAsyncResult, IDisposable")
    /// </summary>
    public interface ITaskMan
    {
        /// <summary>
    ...
    /// <summary>
    /// We're doing this so we can trap & log background errors/exceptions that may occur when a Task is run.
    /// </summary>
    public class TaskMan : ITaskMan
    {
        ...
        void LogError(Exception ex, string origin = "", string filePath = "", int lineNumber = 0)
        {
            try
            {
                using (var fileStream = (TextWriter)new StreamWriter(File.Open(Path.Combine(Directory.GetCurrentDirectory(), "Error.log"), FileMode.Append)))
                {
                    fileStream.WriteLine($"Origin: {origin}, File: {filePath}, Line: {lineNumber}, Exception: {ex.Message}");
                }
            }
            catch { }
        }
